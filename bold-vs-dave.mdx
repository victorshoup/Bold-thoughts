...
Using $P$ \= 1.3 eth, we set $B := \sqrt{3M \times 1.3}$ ~ 2K eth, which leads to $\mathit{Cost}$ \~ 4K eth and $\rho$ \~ (3M)/(4K) \~ 0.75K.
...
Using this, we can bound $\mathit{Cost}$ as follows. Let $A$ be the cost of generating a ZK proof plus the gas cost for verifying such a proof for the entire assertion
