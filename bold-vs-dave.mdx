...
Again with $T$ \= 3M eth, we set $B := \sqrt{3M \times 0.08}$ \~ 0.5K eth  
$\mathit{Cost} \~ 1K eth  
Griefing ratio $\rho$ \~ 3K

Note: the estimate of 0.05 eth for bisections in Dave is based on a sequence of 2^40 blocks, whereas we only need 2^26. So in actuality, it should be a bit less, namely, 0.04 eth, but we will ignore that here.
